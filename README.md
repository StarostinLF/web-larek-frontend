...
- `constructor`: принимает базовый URL API, URL для CDN и дополнительные опции запроса;
- `getProductItem`: метод для получения списка товаров;
- `order`: метод для оформления заказа.
